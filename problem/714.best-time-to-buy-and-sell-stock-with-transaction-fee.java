...
class Solution {
 public int maxProfit(int[] prices, int fee) {
  ...
  buy[0] = c1[0] = -prices[0];
  for (int i = 1; i < n; ++i) {
   buy[i] = Math.max(sell[i - 1] - prices[i], c2[i - 1] - prices[i]);
   sell[i] = Math.max(buy[i - 1] + prices[i] - fee, c1[i - 1] + prices[i] - fee);
   c1[i] = Math.max(buy[i - 1], c1[i - 1]);
   c2[i] = Math.max(sell[i - 1], c2[i - 1]);
  }
  return Math.max(c2[n - 1], sell[n - 1]);
 }
}
// @lc code=end
// 和309一样，注意总是可以cooldown的
